//
//  MainControllerCreditCard.swift
//  IclDashboard
//
...
class MainControllerCreditCard: UIViewController,UITableViewDelegate, UITableViewDataSource {
    ...
    var cekTableBranch = [CekTableBranchCredit]()
    var cekTableCourier = [CekTableCourierCredit]()
    var server = Server()
    ...
    @IBAction func btnCourier(_ sender: Any) {
        ...
        getListCourierCredit()
    }
    
    private func getListBranchCredit(){
        ...
        AF.request(self.server.URL + self.server.URL_CC_PERF_BRANCH, method: .get, parameters: parameters,encoding: URLEncoding.queryString,headers: headers)
            ...
            switch response.result{
                case .success(let value):
                    ...
                    let Persen = json[i]["Persen"].string ?? ""
                        ...
                        self.cekTableBranch.append(CekTableBranchCredit(String(no),BranchName, Kurir,OD,OK, UN,IP,String(format:"%.0f",pers)))
    ...
    private func getListCourierCredit(){
    ...
    //        let Username:String = "fatur.fauzi"
            ...
            AF.request(self.server.URL + self.server.URL_CC_PERF_COURIER, method: .get, parameters: parameters,encoding: URLEncoding.queryString,headers: headers)
                .responseJSON {(response) in
                ...
                switch response.result{
                    case .success(let value):
                        ...
                        let Persen = json[i]["Persen"].string ?? ""
                            ...
                            self.cekTableCourier.append(CekTableCourierCredit(String(no),CourierName, CourierCode,OD,OK, UN,IP,String(format:"%.0f",pers)))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == self.tableBranch {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "TableBranchCredit") as! TableBranchCredit
            cell.setData(data)
            
            return cell
        }else{
            let data2 = cekTableCourier[indexPath.row]
            let cell2 = tableView.dequeueReusableCell(withIdentifier: "TableCourierCredit") as! TableCourierCredit
            cell2.setData(data2)
